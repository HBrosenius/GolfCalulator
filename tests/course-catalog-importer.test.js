...
test('Ölands GK dataset matches its checked-in D1 migration', async () => {
  const { buildMigrationSql, validateVerifiedCourses } = await import('../scripts/build-course-catalog-migration.mjs');
  const courses = JSON.parse(fs.readFileSync('catalog/verified/olands-gk.json', 'utf8'));
  assert.equal(validateVerifiedCourses(courses).length, 1);
  assert.equal(courses.flatMap(course => course.tees.flatMap(tee => tee.ratings)).length, 8);
  const sql = buildMigrationSql(courses);
